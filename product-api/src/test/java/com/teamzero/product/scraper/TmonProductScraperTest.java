...
class TmonProductScraperTest {
  ...
  private static final String TMON_URL = "https://search.tmon.co.kr/search/?keyword=";
  ...
  void getScrapProductList_Tmon() {
    ...
    try {
      ...
      String url = "https://search.tmon.co.kr/api/search/v4/deals?_=1670946124923&keyword=삼성전자 노트북 플러스2 NT550XDA-K14A&mainDealOnly=true&optionDealOnly=false&page=1&showFilter=true&size=60&sortType=POPULAR&thr=hs&useTypoCorrection=true";
      String priceAdd = String.format("&commonFilters=minPrice:%d,maxPrice:%d&thr=hs", minPrice, maxPrice);

      Connection connection = Jsoup.connect(TMON_URL + URLEncoder.encode(keyword, "UTF-8") + priceAdd)
      ...
      Elements eles = document.getElementsByClass("items");
